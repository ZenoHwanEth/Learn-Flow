...
pub contract BottomShot{
  ...
  pub resource NFT {
    // The unique ID that differentiates each NFTs
    pub let id: UInt64

    // We set the ID of the NFT and update the NFT counter
    init(){
      self.id = BottomShot.totalSupply
      // totalSupply is global variable for this contract
  ...
  pub resource Collection {
    // This is a dictionary that maps ID integers with NFT resource
    // the @ indicates that we're working with resource
    ...
    pub fun withdraw(id: UInt64): @NFT{
      ...
      let token <- self.ownedNFTs.remove(key: id) ?? panic("This collection doesn't contain an NFT with that id")
    ...
    init(){
      // All resource value MUST be initiated so we make it empty
      self.ownedNFTs <- {}
    }
